/**
 * Finance Seer - Complete Test Suite v2
 * Includes live IBKR connectivity tests
 */
...
console.log('\n🧪 Finance Seer Complete Test Suite v2\n');
console.log('Running 146 tests (including 5 live IBKR tests)...\n');
...
test('Price change calculation', () => {
  ...
  assert(Math.abs(change - 2.0) < 0.001, 'Price change calculation must be accurate');
});
...
// ===== SECTION 4: IBKR LIVE Connectivity Tests (5 tests) =====
console.log('\n📋 Section 4: IBKR Live Connectivity Tests');
...
// Check if IBKR Gateway is actually running and accepting connections
test('IBKR Gateway port is listening', (done) => {
  ...
  setTimeout(() => {
    if (socket.connecting) {
      throw new Error('IBKR Gateway connection timeout - Gateway may not be running');
    }
  }, 5000);
});

test('IBKR Client module loads successfully', () => {
  // Check if ib_insync is installed by trying to import it in Python
  const { execSync } = require('child_process');
  try {
    const output = execSync('python3 -c "import ib_insync; print(ib_insync.__version__)"', { cwd: process.cwd(), encoding: 'utf8' });
    const version = output.trim();
    assert(version.length > 0, `ib_insync version: ${version}`);
  } catch (e) {
    throw new Error('ib_insync module not installed or not accessible');
  }
});
...
if (failed === 0) {
  ...
  if (failed === 5) {
    console.log('⚠️  ALL IBKR LIVE TESTS FAILED - IBKR Gateway is NOT RUNNING\n');
  }
  process.exit(1);
}
